...
const content = {
  en: {
    ...
    links: [
      { href: '/en/knowledge-hub', label: 'Read the Knowledge Hub' },
  ...
  ar: {
    ...
    links: [
      { href: '/ar/knowledge-hub', label: 'اقرأ مركز المعرفة' },
      { href: '/ar/menu', label: 'استكشف القائمة' },
    ],
  },
}

export function LocaleMusselsAuthority({ locale }: { locale: string }) {
  ...
  return (
    <section className="py-20 px-4" style={{ backgroundColor: '#0D0D0D' }} dir={isRTL ? 'rtl' : 'ltr'}>
      <div className="max-w-6xl mx-auto locale-text-start">
        <div className="max-w-3xl mb-10">
          <h2 className="text-3xl md:text-5xl font-bold leading-tight text-balance" style={{ color: '#F8F6F1', ...headingFont }}>
            {copy.title}
          </h2>
          <p className="mt-6 text-lg leading-8 text-pretty" style={{ color: '#d1d5db', ...bodyFont }}>{copy.intro}</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {copy.blocks.map((block) => (
            <HoverGlow key={block.heading} className="premium-hover-glow rounded-2xl p-6 h-full" style={{ backgroundColor: '#1A1A1A', border: '1px solid rgba(255,255,255,0.08)' }}>
              <h3 className="text-xl font-bold mb-4" style={{ color: '#F8F6F1', ...headingFont }}>{block.heading}</h3>
              <p className="leading-7" style={{ color: '#c9c9c9', ...bodyFont }}>{block.text}</p>
            </HoverGlow>
          ))}
        </div>
        <div className="mt-8 flex flex-wrap gap-3">
          {copy.links.map((link) => (
            <Link key={link.href} href={link.href} className="premium-hover-glow locale-inline rounded-full px-5 py-3 text-sm font-semibold" style={{ color: '#F8F6F1', border: '1px solid rgba(229,30,42,0.35)', backgroundColor: 'rgba(229,30,42,0.08)', ...bodyFont }}>
              {link.label}
