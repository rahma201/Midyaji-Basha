...
export type LocaleOrderModalProps = {
  open: boolean
  onOpenChange: (v: boolean) => void
  dict: ModalDict
}

export function OrderModalInner({ open, onOpenChange, dict }: LocaleOrderModalProps) {
  const isRTL = dict.title === 'اطلب الآن'
  const { arabicFont: directionalFont, headingFont } = getPageFonts(isRTL)
  return (
      ...
      <Dialog.Portal>
        ...
        >
          ...
          <div className="text-center mb-8">
            <h2 className="text-3xl font-bold mb-2" style={{ ...headingFont, color: '#F8F6F1' }}>{dict.title}</h2>
